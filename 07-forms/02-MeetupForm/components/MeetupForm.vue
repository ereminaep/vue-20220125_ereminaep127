...
export default {
  ...
  emits: ['cancel', 'submit', 'remove', 'update:meetup'],

  data() {
    return {
      localMeetup: this.meetup,
      localAgenda: this.meetup?.agenda,
    };
  },
  watch: {
    localMeetup: {
      deep: true,
      handler(newValue) {
        this.$emit('update:meetup', newValue);
      },
    },
  },
  methods: {
  ...
  localCount: {
    deep: true,
    handler(newMeetup) {
      this.$emit('update:meetup', newMeetup);
    },
  },
};
</script>
